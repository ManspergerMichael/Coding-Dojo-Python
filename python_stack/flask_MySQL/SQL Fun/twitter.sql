...
INSERT INTO twitter.users (id, first_name,last_name,handle, created_at,updated_at) VALUES (42,'Billy','Everyteen' ,'Shapopi',now(), now());
UPDATE  twitter.users set first_name = "Gorfalx" WHERE id=42;
SELECT * FROM USERS;
...
#concat_WS
SELECT CONCAT_WS(' ',first_name, last_name, 'smeg head') AS full_name from users;
-- LENGTH()
SELECT LENGTH(last_name) AS last_len FROM users;
#DATE
#HOUR
SELECT MONTH(created_at) as date_month, created_at from users;

#left join
SELECT tweet as JerkBag_tweets
FROM users
LEFT JOIN tweets
